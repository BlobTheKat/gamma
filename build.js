...
const postprocessors = {
	__proto__: null,
	'gamma.js': code =>
		code.replace(/(?<![\w$])gl\.([A-Z][A-Z0-9]*)(?![\w$])/g, (str, name) => glConstants[name] ?? (unknownConstants.size != unknownConstants.add('gl.'+name).size && console.warn('Unminified GL constant: gl.'+name), str))
		.replace(/(?<![\w$])\$\.([A-Z][A-Z0-9]*)(?![\w$])/g, (str, name) => gammaConstants[name] ?? (unknownConstants.size != unknownConstants.add('$.'+name).size && console.warn('Unminified Gamma constant: $.'+name), str))
